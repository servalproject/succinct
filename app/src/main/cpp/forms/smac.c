    ...
    *len_out = 0;
    ...
    if (notRawASCII == 0) {
        ...
        */
        ...
        unsigned char b5to0 = (unsigned char) range_decode_equiprobable(c, 64);
        b5to0 |= (b6 << 6);
        b5to0 |= (b7 << 7);
        m[0] = b5to0;

        // Also stop decoding if we run out of room
        for (i = 1; m[i - 1] && (i < buff_size); i++) {
    ...
    if (notPackedASCII == 0) {
        ...
        *len_out = (unsigned) encodedLength;
        return 0;
    }
    ...
    int alphaPointer = 0;
    unsigned short m16[alphaCount + nonAlphaCount];
    for (i = 0; i < sizeof m16; i++) {
    ...
    if (utf16toutf8(m16, i, m, len_out) == -1)
        return -1;
...
int stats3_compress_radix_append(range_coder *c, unsigned char *m_in, int m_in_len,
                                 stats_handle *h, double *entropyLog) {
    if (m_in_len > NELS(h->messagelengths))
        return -1;
    ...
    range_encode_symbol(c, h->messagelengths, NELS(h->messagelengths), m_in_len);
    return encodePackedASCII(c, m_in);
}

int stats3_compress_model1_append(range_coder *c, const char *m_in, unsigned m_in_len,
                                  stats_handle *h, double *entropyLog) {
    // Message length stats only go up so far...
    if (m_in_len > NELS(h->messagelengths))
        return -1;

    unsigned short utf16[m_in_len];
    unsigned len = sizeof utf16;

    unsigned short alpha[m_in_len]; // message with all non alpha/spaces removed
    unsigned short lcalpha[m_in_len]; // message with all alpha chars folded to lower-case

    // Convert UTF8 input string to UTF16 for handling
    if (utf8toutf16(m_in, m_in_len, utf16, &len))
        return -1;
    ...
    */
    ...
    range_encode_symbol(c, h->messagelengths, NELS(h->messagelengths), len);
...
int stats3_compress_append(range_coder *c, const char *m_in, unsigned m_in_len,
    ...
    range_coder *t1 = range_new_coder(c->bit_stream_length >> 3);
    ...
    range_coder *t2 = range_new_coder(c->bit_stream_length >> 3);
...
int stats3_compress_bits(range_coder *c, const char *m_in, unsigned m_in_len,
                         stats_handle *h, double *entropyLog) {
    if (stats3_compress_append(c, m_in, m_in_len, h, entropyLog))
        return -1;
    range_conclude(c);

...
/*
  Re-constitute a form to XML (or other format) by reading a template of the
  output format and substituting the values in.

  Update 26.05.2016 : Stripped2xml supports forms with subforms (But only 1
  level of subforms!)
*/
int stripped2xml(char *stripped, int stripped_len, char *template,
                 int template_len, char *xml, int xml_size) {
  int xml_ofs = 0;
  int state = 0;
  int i, j, k;
  ...
  for (i = 0; i < stripped_len; i++) {
    if (stripped[i] == '=' && (state == 0)) {
      ...
      //{Opening bracket} X {We are not already in a subform} => beginning of
      //subform
      if (state_in_subform_instance == 0) {
        state_in_subform_instance = 1;
      }
      //{Opening bracket} X {We are already in a subform} => beginning of
      //subform record
      else {
      ...
      //{Closing bracket} X {We are already in a subform record} => end of
      //subform record
      if (state_in_subform_record == 1) {
        state_in_subform_record = 0;
      }
      //{Closing bracket} X {We are not already in a subform record} => end of
      //subform
      else {
  ...
  for (i = 0; i < template_len; i++) {
    ...
    if (template[i] == '<') {
      ...
      else if (!strncasecmp("data", tag, strlen("data")) &&
               state_in_subform_instance == 1) {
      ...
      else if ((!strncasecmp("data", &tag[1], strlen("data")) &&
                state_in_subform_instance == 1) &&
               tag[0] == '/') {
        printf("Found a data END tag within a subform ! \n");
        if (remaining_records == 1) {
          number_of_records_written++;
          // set remaining_records to 0, it will be set to 1 if we have another
          // record
          remaining_records = 0;
      ...
      if (state_field == 1) {
        ...
        for (j = 0; j < field_count; j++)
          if (fieldnames[j] && !strcasecmp(field, fieldnames[j])) {
            ...
            if (in_subform_record[j] == 1) {
              // If we have written the total number of records (i.e. count -
              // written == 1) we dont set remaining records to 0
              if ((record_count[record_number_field[j]] -
            ...
            // will find the other record fieldnames if the fields have the same
            // name for example "uuid" or fieldnames in the records of a same
            // subform
	    free(fieldnames[j]);
            fieldnames[j] = NULL;
            break;
  ...
  while (c >= -1 && (xmlofs < xml_len)) {
    ...
    case '<':
      ...
      if (interesting_tag && val_len > 0) {
        value[val_len] = 0;
        // Magpi puts ~ in empty fields -- don't include these in the stripped
        // output
        ...
        /* End added code */
          ...
          int b = snprintf(&stripped[stripped_ofs],
                           stripped_size - stripped_ofs, "%s=%s\n", tag, value);
          if (b > 0)
            stripped_ofs += b;
        }
        val_len = 0;
        ...
        if (!form_name) {
          ...
          */
          ...
          if (!strncasecmp("dd:subform", tag, strlen("dd:subform"))) {
            ...
            in_subform++;
            printf(
                "Test-Dialog / dexml.c : Found start of subform ! Tag = %s \n",
                tag);
            int b = snprintf(&stripped[stripped_ofs],
                             stripped_size - stripped_ofs, "{\n");
            if (b > 0)
              stripped_ofs += b;
          }
          ...
          if ((!strncasecmp("dd:subform", &tag[1], strlen("dd:subform"))) &&
            ...
            in_subform--;
            printf(
                "Test-Dialog / dexml.c : Reached end of subform ! Tag = %s \n",
                tag);
            int b = snprintf(&stripped[stripped_ofs],
                             stripped_size - stripped_ofs, "}\n");
            if (b > 0)
          ...
          if ((!strncasecmp("data", &tag[1], strlen("data"))) &&
              (tag[0] == '/') && (in_subform)) {
            int b = snprintf(&stripped[stripped_ofs],
                             stripped_size - stripped_ofs, "}\n");
            if (b > 0)
          ...
          if (!in_instance) {
            // ODK form name appears as attributes of a tag which has a named
            // based on the name of the form.
            char name_part[1024];
            char version_part[1024];
            int r = 0;
            if (strlen(tag) < 1024) {
              r = sscanf(tag, "%s id=\"%[^\"]\" version=\"%[^\"]\"", exit_tag,
                         name_part, version_part);
            }
            if (r == 3) {
              // Add implied formid tag for ODK forms so that we can more easily
              // find the recipe that corresponds to a record.

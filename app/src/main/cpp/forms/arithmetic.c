...
static int range_check(range_coder *c, int line);
static int range_rescale(range_coder *c);
static int range_calc_new_range(range_coder *c,
                         unsigned int p_low, unsigned int p_high,
                         unsigned int *new_low, unsigned int *new_high);
static int range_decode_common(range_coder *c, unsigned int p_low, unsigned int p_high, int s);
...
static int range_decode_getnextbit(range_coder *c) {
...
static int range_emitbit(range_coder *c, int b) {
...
static int range_emit_stable_bits(range_coder *c) {
...
/* If there are underflow bits, squash them back into 
   ...
   bits. */
static int range_unrescale_value(unsigned int v, int underflow_bits) {
    int i;
...
static char *_asbits(char *bitstring, unsigned int v) {
    int i;
    bitstring[32] = 0;
    for (i = 0; i < 32; i++)
        if ((v >> (31 - i)) & 1)
...
#define asbits(V) _asbits(alloca(33), V)

static unsigned long long range_space(range_coder *c) {
...
int range_encode(range_coder *c, unsigned int p_low, unsigned int p_high) {
    ...
    if (c->debug)
        LOGE("Calculating new_low and new_high from p_low=0x%x, p_high=0x%x\n",
             p_low, p_high);
...
static int range_equiprobable_range(range_coder *c, int alphabet_size, int symbol, unsigned int *p_low,
                             unsigned int *p_high) {
    *p_low = (((unsigned long long) symbol) * 0xffffff) / (unsigned long long) alphabet_size;
    *p_high = (((1LL + (unsigned long long) symbol) * 0xffffff) /
...
int range_encode_equiprobable(range_coder *c, int alphabet_size, int symbol) {
    ...
    if (c->debug)
        LOGE("Encoding %d/%d: p_low=0x%x, p_high=0x%x\n",
...
int range_decode_equiprobable(range_coder *c, int alphabet_size) {
    ...
    if (c->debug)
        LOGE("decoding: alphabet size = %d, estimating s=%d (0x%x)\n",
             alphabet_size, s, s);
    ...
    for (symbol = s ? s - 1 : 0; symbol < s + 2 && symbol < alphabet_size; symbol++) {
        ...
        if (!range_decode_common(c, p_low, p_high, symbol)) {
            if (c->debug)
                LOGE("Decoding %d/%d p_low=0x%x, p_high=0x%x\n",
                     symbol, alphabet_size, p_low, p_high);
            return symbol;
        }
    }
...
static char *_range_coder_lastbits(char *bitstring, range_coder *c, int count) {
    ...
    for (i = (c->bits_used - count); i < c->bits_used; i++) {
        int byte = i >> 3;
        int bit = (i & 7) ^7;
        bitstring[l++] = (c->bit_stream[byte] & (1 << bit)) ? '1' : '0';
    }
    bitstring[l] = 0;
    return bitstring;
}

#define range_coder_lastbits(C,L) _range_coder_lastbits(alloca(L+1),C,L)

static int range_status(range_coder *c, int decoderP) {
    unsigned int value = decoderP ? c->value : (
            ((unsigned long long) c->high + (unsigned long long) c->low) >> 1LL);
    unsigned long long space = range_space(c);
    if (!c) return -1;
...
int range_conclude(range_coder *c) {
    ...
    if (c->debug) {
        LOGE("after shifting out msb and underflow bits: low=0x%x, high=0x%x\n",
             c->low, c->high);
        range_status(c, 0);
    }
...
static int range_coder_reset(struct range_coder *c) {
    c->low = 0;
    c->high = 0xffffffff;
    c->entropy = 0;
    c->bits_used = 0;
...
struct range_coder *range_new_coder(int bytes) {
    ...
    c->bit_stream = (bytes) ? malloc(bytes) : NULL;
...
static int range_check(range_coder *c, int line) {
    if (c->low >= c->high) {
        if (!line) return -1;
        LOGE("c->low >= c->high at line %d", line);
        return -1;
    }
    ...
    if (c->value > c->high || c->value < c->low) {
        if (!line) return -1;
        LOGE("c->value out of bounds %d", line);
...
int range_decode_symbol(range_coder *c, unsigned int frequencies[], int alphabet_size) {
        ...
        if (c->value < boundary) {
            if (c->debug) {
                ...
                if (s > 0) {
                    ...
                    LOGI("  previous boundary @ 0x%08x", boundary);
                }
            }
            break;
        } else {
    ...
    if (c->debug)
        LOGI("s=%d, value=0x%08x, p_low=0x%08x, p_high=0x%08x",
...
static int range_calc_new_range(range_coder *c,
                         ...
                         unsigned int *new_low, unsigned int *new_high) {
    unsigned long long space = range_space(c);
    if (c->debug) LOGE("calculating new range using space=0x%llx", space);

    if (space < MAXVALUEPLUS1) {
        c->errors++;
        if (c->debug) LOGI("%s : ERROR: space(0x%08llx)<0x%08x", c->debug, space, MAXVALUEPLUS1);
        return -1;
    }
    ...
    if (c->debug) LOGE("(0x%x * 0x%llx)>>24 = 0x%llx", p_low, space, (p_low * space) >> 24LL);
...
static int range_decode_common(range_coder *c, unsigned int p_low, unsigned int p_high, int s) {
    ...
    if (range_check(c, 0 /* don't abort if things go wrong */)) {
        if (c->debug) LOGE("range check failed at %s:%d", __FILE__, __LINE__);
        return -1;
    }

    if (range_calc_new_range(c, p_low, p_high, &new_low, &new_high)) {
        if (c->debug) LOGE("range calc new range failed at %s:%d", __FILE__, __LINE__);
        return -1;
    }

    if (new_high > 0xffffffff) {
        LOGI("new_high=0x%08x", new_high);
        new_high = 0xffffffff;
    }

    if (0) {
        LOGI("rdc: low=0x%08x, value=0x%08x, high=0x%08x", c->low, c->value, c->high);
    ...
    if (new_low > c->value || new_high < c->value) {
        if (c->debug) {
            LOGE("c->value would be out of bounds at %s:%d", __FILE__, __LINE__);
            LOGE("  new_low=0x%08x, c->value=0x%08x, new_high=0x%08x", new_low, c->value,
                 new_high);
            LOGE("  low=0x%08x, value=0x%08x, high=0x%08x", c->low, c->value, c->high);
            LOGE("  p_low=0x%08x, p_high=0x%08x, space=%08llx, s=%d",
                 p_low, p_high, range_space(c), s);
            LOGE("  c->low=0x%x, c->high=0x%x", c->low, c->high);
    ...
    if (c->debug)
        LOGI("%s: after decode: low=0x%08x, high=0x%08x",
    ...
    if (c->debug)
        LOGI("%s: after rescale: low=0x%08x, high=0x%08x",
             c->debug, c->low, c->high);
